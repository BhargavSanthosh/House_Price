...
#loading the model file pickle
model=pickle.load(open('House_Price.pkl','rb'))

#creating function to accept inputs and creating 2d array and predicting the result
def predict(bedrooms,bathrooms,status,size,location,facing,Types):
    ...
    input_data=np.array([[bedrooms,bathrooms,Selected_location,size,Selected_status,Selected_facing,Selected_type]])
    result=model.predict(input_data)[0].round(2)
    return result

if __name__=="__main__":
    st.header("House Price Prediction")
    #st.title("The King's Pawn")
